...
/**
 * Example of using SPI_ADP to write "Hello World" using the IO abstraction.
 */
static uint8_t example_SPI_ADP[12] = "Hello World!";

void SPI_ADP_example(void)
{
	struct io_descriptor *io;
	spi_m_sync_get_io_descriptor(&SPI_ADP, &io);

	spi_m_sync_enable(&SPI_ADP);
	io_write(io, example_SPI_ADP, 12);
}
